...
이 단계는 기본적으로 현재 브라우저에 저장되는 앱 데이터를, 필요할 때 Supabase 클라우드에 직접 저장하고 불러오기 위한 준비입니다.
...
## 5. 앱에서 수동으로 저장하고 불러오기

1. 앱의 `공유` 탭으로 갑니다.
2. `클라우드 동기화`에서 이메일과 비밀번호를 입력합니다.
3. 처음이면 `가입`, 이미 가입했다면 `로그인`을 누릅니다.
4. 다른 기기나 브라우저에서 이어 보려면 `클라우드에 저장`을 눌러 현재 데이터를 직접 저장합니다.
5. 같은 계정으로 저장한 데이터를 현재 기기로 가져오려면 `클라우드에서 불러오기`를 누릅니다.
6. 불러오기를 실행하면 현재 브라우저의 데이터가 클라우드 데이터로 교체됩니다.

## 중요한 점

Supabase는 데이터를 인터넷에 저장하는 곳입니다. 이 앱은 자동 동기화가 아니라 사용자가 버튼을 눌러 저장하고 불러오는 방식입니다. 사이트 주소를 외부에 공개하려면 나중에 Netlify, Vercel, GitHub Pages 같은 배포 단계가 한 번 더 필요합니다.
